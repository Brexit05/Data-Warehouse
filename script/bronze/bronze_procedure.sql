/* 
The bronze.load_bronze stored procedure is responsible for automating the data ingestion process into the bronze layer of the data warehouse. It begins by printing progress messages for visibility and logging, then sequentially truncates existing data in each bronze table to ensure a clean load. Using BULK INSERT, it efficiently loads raw data from CSV files located in both source_crm and source_erp directories into their corresponding tables, including customer, product, sales, and ERP records. The procedure also measures execution time to monitor performance and includes structured error handling via a TRY...CATCH block to catch any failures during the loading process. This operation ensures that the bronze layer always reflects the most up-to-date raw data from the source systems.*/
